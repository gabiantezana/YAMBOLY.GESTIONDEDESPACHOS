...
namespace SAPADDON.FORM._MSS_ASUAForm
{
    public class MSS_ASUAForm : BaseApplication, ISAPForm
    {
        private Form _Form { get; set; }
        public Form GetForm() => _Form;
        public string GetFormId() => FormID.MSS_ASUA.IdToString();
        public static string GetFormName() => "Asignación de almacén a usuario";

        public MSS_ASUAForm(Dictionary<string, ISAPForm> dictionary)
        {
            if (FormTypeWasOpened(GetType())) return;

            _Form = SapFormHelper.CreateForm(GetApplication(), XMLHelper.GetXMLString(EmbebbedFileName.MSS_ASUAForm), GetFormId());
        ...
        private void FillMatrix()
        {
            ...
            /*
            if (GetMatrix().RowCount > 0)
            {
                for (var i = 1; i <= GetMatrix().RowCount; i++)
                {
                    GetMatrix().Columns.Item(0).Cells.Item(i).Specific.Value = i.ToString();
                }
        ...
        {
            var chooseFromListcolumnNameToGet = string.Empty;
            var dataSourceColumnName = string.Empty;

            if (chooseFromListEvent.ChooseFromListUID == FormItemIds.cflUsers.IdToString())
            {
                chooseFromListcolumnNameToGet = "USER_CODE";
                dataSourceColumnName = new MSS_ASUA().GetMemberName(x => x.MSS_USUA);
            }

            else if (chooseFromListEvent.ChooseFromListUID == FormItemIds.cflWareHouses.IdToString())
            {
                chooseFromListcolumnNameToGet = "WhsCode";
                dataSourceColumnName = new MSS_ASUA().GetMemberName(x => x.MSS_ALMA);
            }

            var valueToSet = chooseFromListEvent.SelectedObjects.GetValue(chooseFromListcolumnNameToGet, 0);
            var form = GetApplication().Forms.GetFormByTypeAndCount(chooseFromListEvent.FormType, chooseFromListEvent.FormTypeCount);
        ...
        private List<MSS_ASUA> GetItemList()
        {
            var list = new List<MSS_ASUA>();
            var rowCount = GetMatrix().VisualRowCount;
            for (var i = 1; i <= rowCount; i++)
            {
                list.Add(new MSS_ASUA()
                {
                    MSS_USUA = GetMatrix().Columns.Item(1).Cells.Item(i).Specific.Value,
                    MSS_ALMA = GetMatrix().Columns.Item(2).Cells.Item(i).Specific.Value,
                });
            }
            return list;
        }

        public static void SaveUDO(MSS_ASUA item)
        {
            var mService = GetCompany().GetCompanyService().GetGeneralService(nameof(MSS_ASUA));
            var generalData = mService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData) as GeneralData;

            //generalData.

            var code = item.MSS_USUA + item.MSS_ALMA;
            ...
            generalData.SetProperty(item.GetMemberName(x => x.MSS_ALMA), item.MSS_ALMA);
        ...
        {
            var items = DoQuery(EmbebbedFileName.MSS_ASUA_UDO_GetList);
        ...
        {
            var oGeneralService = GetCompany().GetCompanyService().GetGeneralService(nameof(MSS_ASUA));
